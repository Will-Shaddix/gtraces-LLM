...
bool
PolicyManager::recvTimingReq(PacketPtr pkt)// problem in here
{
    ...
    if (ORB.size() >= orbMaxSize) {
        ...
        if (pkt->isRead()) {
            polManStats.numRdRetry++;
            DPRINTF(PolicyManager, " Retrying!\n");
        }
        else {
            polManStats.numWrRetry++;
        }
    ...
    DPRINTF(PolicyManager, "Policy manager accepted packet %lld  ORB Size: %d  max size = %d\n", pkt->getAddr(), ORB.size(), orbMaxSize);
...
PolicyManager::setNextState(reqBufferEntry* orbEntry)
{
    ...
    ////////////////////////////////////////////////////////////////////////
    /// FirstTouch
    /// This system is assuming a hybrid memory without a cache that acts on a fcfs basis for "near memory" 
    /// Basic flow, check near memory table(NMT)
    /// on a write:
    ///     is addr in NMT?
    ///         yes: write loc memory
    ///         no: is NMT full?
    ///             yes: write far memory
    ///             no: write near memory, update NMT



    if (orbEntry->pol == enums::FirstTouch &&
        orbEntry->state == start) {
            //DPRINTF(PolicyManager, "set: waitingLocMemWriteResp -> NONE : %d\n", orbEntry->owPkt->getAddr());
            bool flag = false;
            for(int i = 0; i < NMT.size(); i++){
                flag = NMT[i].contains(orbEntry->owPkt->getAddr());
                if (flag == true){
                    break;
                }
            } 

            if(flag == false && NMT.size() < NMT_Max_Entries){ // if NMT is not full and current addr is not in near memory, add it to near memory
                Addr temp = orbEntry->owPkt->getAddr() >> 2;
                temp = temp << 2;
                NMT.push_back(AddrRange(temp, temp+3));
                flag = true;
            }

            if(flag == true && orbEntry->owPkt->isRead()) {//in local memory and is read
                orbEntry->state = locMemRead;
                orbEntry->locRdEntered = curTick();
            }
            else if(flag == true && orbEntry->owPkt->isWrite()){// in local memory and is write
                orbEntry->state = locMemWrite;
                orbEntry->locWrEntered = curTick();
            }
            else if(flag == false && orbEntry->owPkt->isRead()) {
                orbEntry->state = farMemRead;
                orbEntry->farRdEntered = curTick();
            }
            else if(flag == false && orbEntry->owPkt->isWrite()){
                orbEntry->state = farMemWrite;
                //orbEntry->farWrEntered = curTick(); //no such thing has farWrEntered
            }
            return;
    }
}
...
PolicyManager::handleNextState(reqBufferEntry* orbEntry)
{
    ...
    if (orbEntry->pol == enums::FirstTouch &&
        ...
        pktFarMemWriteHybrid.push_back(orbEntry->owPkt->getAddr());

        polManStats.avgFarWrQLenEnq = pktFarMemWriteHybrid.size();
...
{
    DPRINTF(PolicyManager, "in resume Conflicting!\n");
...
bool
PolicyManager::ReqPortPolManager::recvTimingResp(PacketPtr pkt) // PROBLEM HERE With Multiple policy managers
{
    // if (this->name() == "system.mem_ctrl.loc_req_port") {
