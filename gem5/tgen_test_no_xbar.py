...
args.add_argument(
    "mem_type",
    type=str,
    help="LLM or LL_NVM_PCM or LL_NVM_PCM_3 or LL_NVM_PCM2 or DDR or HBM",
)
...
bpc = 1
if(options.mem_type != "DDR" and options.mem_type != "HBM"):
    bpc = 32
...
if(options.mem_type != "DDR" and options.mem_type != "HBM"):
    system.scheds = [MemScheduler(read_buffer_size = 10, write_buffer_size = 32, resp_buffer_size = 64, unified_queue = options.unified_queue, \
...
for i in range (0,num_LLM_chnls):
    ...
    if(options.mem_type == "DDR"):
        system.mem_ctrlrs[i].dram =DDR4_2400_16x4(range=AddrRange(curr_mem, next_mem), in_addr_map=True, null=True)
    if(options.mem_type == "HBM"):
        system.mem_ctrlrs[i].dram =HBM_2000_4H_1x64(range=AddrRange(curr_mem, next_mem), in_addr_map=True, null=True)
        
    if(options.mem_type != "DDR" and options.mem_type != "HBM"):
        system.scheds[int(i / bpc)].mem_side = system.mem_ctrlrs[i].port
    else:
        system.mem_ctrlrs[i].port = system.membus.mem_side_ports
...
if(options.mem_type != "DDR" and options.mem_type != "HBM"):
    for i in range(options.num_chnls):
        system.scheds[i].cpu_side = system.membus.mem_side_ports 
